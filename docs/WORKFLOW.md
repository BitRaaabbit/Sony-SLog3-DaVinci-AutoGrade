...
Production uses a verified-project clone boundary instead of reconstructing RCM in each fresh project. Load the exact machine-verified Reference Project, read back Project Format and all ten RCM fields without setters, save it, export a timestamped DRP to ASCII temporary storage, record its size and SHA-256, and import it under a new unique production name. Repeat the complete read-only verification in the clone; only `PRODUCTION_RCM_INHERITED_VERIFY=ALL_MATCH` permits compatibility transcoding. Preserve all inherited reference media/timelines/Gallery state and name new timelines `PROD_<STEM>_NEUTRAL_SAFE`.

Long compatibility transcodes and final hard validation run outside Resolve. External pretranscode writes `<stem>.partial.mov`, validates codec, pixel format, geometry, FPS, frame count, duration, PCM audio, range, size, and SHA-256, then atomically renames to the declared working path and emits a private all-PASS manifest. Resolve imports only those prepared files, applies the locked DRX, and requires Render Job completion. Because internal Lua file I/O is not a reliable Unicode-path authority on Windows, external postflight proves target existence, size, ffprobe properties, and SHA-256, then deletes a working file only after its matching Final Master passes.

Windows path identity is established externally with `.NET Path.GetFullPath()` and explicit UTF-8 serialization. The prepared manifest records `canonical_original_path`; private runtime records the same authority as `canonical_source_path`. Resolve compares those canonical values and retains stem/frame/duration/working-SHA gates. Never regenerate media merely because a noncanonical or mojibake path string failed comparison.
