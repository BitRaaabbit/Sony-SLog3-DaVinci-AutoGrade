...
7. resolves the first Original → Working mapping, proves both declared files are visible through Resolve MediaStorage, imports only the exact declared working path when compatibility media is required, and evaluates the effective input policy from the corresponding original metadata without attempting undocumented per-clip Input Color Space writes;
...
For compatibility media, success additionally requires `ORIGINAL METADATA=VERIFIED`, `WORKING MEDIA MAPPING=VERIFIED`, `WORKING MEDIA IMPORT=PASS`, `WORKING_MEDIA_VIDEO_DECODE=PASS`, and `TIMELINE=ONE_EXACT_WORKING_SOURCE`. The final diagnostic status is `SUCCESS_DNXHR_COMPATIBILITY_READY`. AutoGrade rejects a required working-media mapping until both `signal_equivalence_status` and `resolve_decode_status` are explicitly `PASS`; once enabled, it imports the mapped working path but keeps output naming and color policy tied to the original source identity.
